# Functional Use Case

Outlines the main functional scenarios that describe how users interact with Kya Saabzi’s core features.

---

### Use Case 1: Sign-Up {data-toc-label="Sign-Up"}

| Element            | Detail  |
|:-------------------|:--------|
| **Name**           | Sign Up
|  **ID**            | UC-01
| **Description**    | User can register themself from here
| **Actors**         | User, System
| **Pre-Conditions** | <ul><li>Sign Up Page with `Username`, `Email` & `Password` fields</li></ul>
|  **Basic Flow**    | <ul><li> User selects `Username` field and enters preferred username</li><li> User selects `Email` field and enters email </li><li> User selects the `Password` field and enters password</li><li> User clicks on the `Register` button </li><li> System stores the `username` & `password` in the database </li></ul>
| **Post-Conditions**| User has been successfully registered

### Use Case 2: Login {data-toc-label="Login"}

| Element            | Detail  |
|:-------------------|:--------|
| **Name**           | Login
|  **ID**            | UC-02
| **Description**    | User can login from here
| **Actors**         | User, System
| **Pre-Conditions** | <ul><li>Login Page with `Email` & `Password` fields</ul></li>
|  **Basic Flow**    | <ul><li> User selects `Email` field and enters registered username</li><li> User selects `Password` field and enters their password </li><li> User clicks on the `Login` button </li><li> System authenticates the user and logs them in </li></ul>
| **Post-Conditions**| User has been successfully logged in.

### Use Case 3: Add dish {data-toc-label="Add Dish"}

| Element            | Detail  |
|--------------------|---------|
| **Name**           | Add the dish
|  **ID**            | UC-03
| **Description**    | User can post what they have cooked today with an optional note
| **Actors**         | User, System
| **Pre-Conditions** | <ul><li>User has been identified</li><li>Interface to post a dish is present</li></ul>
|  **Basic Flow**    | <ul><li> User enters the dish cooked today (with an optional note)</li><li> User presses the `Submit` button </li><li> System stores the dish and log in the database </li></ul>
| **Post-Conditions**| Dish cooked today is stored.

### Use Case 4: Get recommendation {data-toc-label="Get Recommendation"}

| Element            | Detail  |
|--------------------|---------|
| **Name**           | Dish Recommendation
|  **ID**            | UC-04
| **Description**    | User can get dish recommendation based on their history and other's preferences
| **Actors**         | User, System
| **Pre-Conditions** | <ul><li>Others must have cooked atleast one dish</li></ul>
|  **Basic Flow**    | <ul><li>User gets upto three recommendations with notes by community</li><li> User clicks on the `Get recommendation` button </li></ul>
| **Post-Conditions**| Recommendations were successfully provided

### Use Case 5: Get dish logs {data-toc-label="Cooklogs"}

| Element            | Detail  |
|--------------------|---------|
| **Name**           | Cooklogs
|  **ID**            | UC-05
| **Description**    | User can see the past 7 dishes they cooked
| **Actors**         | User, System
| **Pre-Conditions** | <ul><li>Users must have cooked atleast one dish</li></ul>
|  **Basic Flow**    | <ul>User clicks on `View My Cooklogs` button</ul>
| **Post-Conditions**| Logs of the dishes cooked are provided

### Use Case 6: Delete dish logs {data-toc-label="Delete Dish Logs"}

| Element            | Detail  |
|--------------------|---------|
| **Name**           | Delete dish logs
|  **ID**            | UC-06
| **Description**    | User can delete the logs for a dish
| **Actors**         | User, System
| **Pre-Conditions** | <ul><li>Users must have cooked atleast one dish</li></ul>
|  **Basic Flow**    | <ul><li>User clicks on the `Delete` button</li></ul>
| **Post-Conditions**| Log for the dish is deleted
